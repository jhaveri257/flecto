...
if user == None:
    file_path = f"/ETTUS-data-collection/aioquic/logs/aioquic.log"
else:
    file_path = f"/home/{user}/ETTUS-data-collection/aioquic/logs/aioquic.log"
...
class QuicLoggerTrace:
    ...
    def log_event(self, *, category: str, event: str, data: Dict) -> None:
        if category == "recovery":
            ...
            if data.get("cwnd") is not None and self._last_received.get("cwnd") != data.get("cwnd") and len(data) == 7:
                f.write(f"{int(time.time())}, {data}\n")
                f.flush()
                self._last_received = data
        self._events.append(
            {
